...
In GluCoEnv, two types of observation-spaces are provided. The default observation-space named  :guilabel:`current` outputs the current glucose sensor measurement, while :guilabel:`past_history` provides an observation-space where glucose sensor and insulin pump measurements are augmented a window of past historical values. The :guilabel:`past_history` is useful when formulating the problem as a Partially Observable Markov Decision Process (POMDP), while :guilabel:`current` is used in many open-loop clinical treatment algorithms and classical algorithms like PID and MPC.
